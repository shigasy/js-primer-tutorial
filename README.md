...
- オブジェクト型は参照型。値そのものではなく値への参照を経由して操作されるため。値ではなく、アドレスのやつ。
...
- 安易に!!によるbool変換に頼るより、明示的に出来る方法 str.length > 0 などにした方が可読性が高まる
...
## オブジェクト
- オブジェクトのプロパティ名は`""`や`'`を省略できるが、変数の識別子として利用できないプロパティは`""`や`'`で囲む必要がある。
...
- in演算子はオブジェクト自身が持っていなければ、そのオブジェクトの継承元であるprototypeオブジェクトまで探索する。だからfor...inはNGってことかな
...
- 例外としてObject.create(null)でObject.prototypeを継承しない、プロパティやメソッドをまったく持たない本当にからのオブジェクトが作れる
...
- JavaScriptには、リテラルを使ったプリミティブ型の文字列と、ラッパーオブジェクトを使った文字列オブジェクトがある（真偽値や数値も同様）
  - 使い分ける利点はないため、常にリテラルを使うことを推奨
    ...
    - インスタンスを扱う利点がそもそもない
    - typeof演算子でラッパーオブジェクトを評価するとobjectとなり混乱する
...
- Mapのように活用するオブジェクトの利点
  ...
  - ライブラリやAPIがオブジェクトのような形で返ってくる（ES2015以前の名残とか簡単さからだと思われる）
...
- 副作用のためのインポート
  ...
  - これが、importしたら、勝手に実行された正体（基本的にグローバルコードは実行されそう）
```javascript
window.foo = "foo";
```
```javascript
...
## TODO
- [x] Symbol何に使う
  - [x] symbolはES2015で追加された新しいプリミティブ型
  - [x] 作成する度にまったく新しいSymbolを返す
  - [x] 二度とまったく同じsymbolを作ることができない
  - [x] オブジェクトのキーとして利用することができる
    - [x] キーがかぶらない
    - [x] 衝突を防げる
  - [x] JavaScriptの実行環境は世界中で勝手にどんどんアップデートされていく
  - [x] 互換性があるように言語仕様を導入する仕組み
  - [x] ビルトインクラスの安全な拡張とかできる
- [x] ビルトインオブジェクト is 何
...
- [x] ビルトインオブジェクトの拡張
  - [x] Array.prototype拡張より、Arrayを継承したMyArrayクラスなどを作ったほうがライブラリとの衝突や影響範囲が狭いと思う
- [x] strictモードに自動でなる条件
  - [x] "use strict"と書く
  - [x] 最初のJavaScriptファイルの冒頭でstrictモードの宣言があり、結合された場合
  - [x] type="module"としてモジュールとして読み込まれたコードは、全体が強制的にStrict Modeになる
  - [x] Babelでes2015に含まれるes2015-modules-commonjsプラグインでなるらしい
- [x] プロトタイプオブジェクト
  - [x] 参照: ./prototype.js
- [x] this
  - [x] 参照: ./this.js
